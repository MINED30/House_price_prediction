...
@userbp.route('/delete/')
@userbp.route('/delete/<deleteid>')
def delete_replies(deleteid):
    if 'userid' in session:
        userid = session.get('userid', None)
        repl_todel = Reply.query.filter(Reply.reply_id==deleteid).first()
        if userid == repl_todel.userid:
            db.session.delete(repl_todel)
            db.session.commit()
            return redirect(url_for('main.home',msg_code=11))
        else:
            return redirect(url_for('main.home',msg_code=10))
